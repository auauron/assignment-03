...
app.post(["/api/signup", "/signup"], (req, res) => {
  ...
  if (!isValidEmail) {
    return res.status(400).json({ success: false, message: "Email must contain @ and end with .com" });
  }
...
app.post(["/api/login", "/login"], (req, res) => {
  ...
  return res.json({ success: true, message: "Login successful.", user: { firstname: user.firstname, lastname: user.lastname, email: user.email, birthdate: user.birthdate } });
});
